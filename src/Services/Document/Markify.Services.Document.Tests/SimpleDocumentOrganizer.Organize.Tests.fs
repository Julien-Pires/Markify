...
module BasicDocumentationOrganizerTests =
    ...
    let ``Organize should return correct pages count`` (expected, setting, root, library, sut : SimpleDocumentOrganizer) =
        let actual = (sut :> IDocumentOrganizer).Organize library root setting
    ...
    [<DocumentData("FooProject", 0, 0, "md")>]
    let ``Organize should return correct path`` (setting, root, library, sut : SimpleDocumentOrganizer) =
        let actual = (sut :> IDocumentOrganizer).Organize library root setting
    ...
    let ``Organize should return page with correct name`` (expected, name, setting, root, library, sut : SimpleDocumentOrganizer) =
        let toc = (sut :> IDocumentOrganizer).Organize library root setting
    ...
    let ``Organize should not return page with incorrect name`` (expected, setting, root, library, sut : SimpleDocumentOrganizer) =
        let toc = (sut :> IDocumentOrganizer).Organize library root setting
    ...
    let ``Organize should return page with correct path`` (path, expected, setting, root, library, sut : SimpleDocumentOrganizer) =
        let toc = (sut :> IDocumentOrganizer).Organize library root setting
