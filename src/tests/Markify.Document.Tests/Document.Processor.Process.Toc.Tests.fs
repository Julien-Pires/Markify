module Document_Processor_Process_Toc_Tests
    ...
    let ``Process library should return correct pages count`` (expected, setting, library, sut : IDocumentOrganizer) =
        let actual = sut.Organize (library, setting)
    ...
    let ``Process library should return correct path`` (expected, setting, library, sut : IDocumentOrganizer) =
        let actual = sut.Organize (library, setting)
