module Document_Processor_Process_Toc_Tests
    ...
    let ``Process library should return correct pages count`` (expected, setting, solution, library, sut : IDocumentationOrganizer) =
    ...
    let ``Process library should return correct path`` (expected, setting, solution, library, sut : IDocumentationOrganizer) =
        let actual = sut.Organize (library, solution, setting)
